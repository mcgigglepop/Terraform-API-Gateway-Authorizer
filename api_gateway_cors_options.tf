...
# CORS OPTIONS Method for Update Endpoint
resource "aws_api_gateway_method" "cors_method_update" {
  rest_api_id   = aws_api_gateway_rest_api.apigw_api_gateway.id
...
# CORS Integration for Update Endpoint
resource "aws_api_gateway_integration" "cors_integration_update" {
  rest_api_id = aws_api_gateway_rest_api.apigw_api_gateway.id
  resource_id = aws_api_gateway_resource.update_resource.id
  http_method = aws_api_gateway_method.cors_method_update.http_method
  type                    = "MOCK"
  request_templates = {
    "application/json" = <<EOF
{ "statusCode": 200 }
...
# CORS Method Response for Update Endpoint
resource "aws_api_gateway_method_response" "cors_method_response_update" {
  rest_api_id = aws_api_gateway_rest_api.apigw_api_gateway.id
  resource_id = aws_api_gateway_resource.update_resource.id
  http_method = aws_api_gateway_method.cors_method_update.http_method
...
# CORS Integration Response for update Endpoint
resource "aws_api_gateway_integration_response" "cors_integration_response_update" {
  rest_api_id = aws_api_gateway_rest_api.apigw_api_gateway.id
  resource_id = aws_api_gateway_method.cors_method_update.resource_id
  http_method = aws_api_gateway_method.cors_method_update.http_method

  status_code = aws_api_gateway_method_response.cors_method_response_update.status_code
