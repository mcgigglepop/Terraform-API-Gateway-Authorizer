...
resource "aws_lambda_function" "put_lambda" {
  ...
  function_name     = "${var.SERVICE}-${var.STAGE}-put"
  role          = aws_iam_role.put_lambda_role.arn
  handler       = "functions/put.put"
  runtime       = "nodejs12.x"
  environment {
    variables = {
...
# Cloudwatch Log Group Resource for the Put Function
resource "aws_cloudwatch_log_group" "put_lambda_cw_group" {
  name              = "/aws/lambda/${aws_lambda_function.put_lambda.function_name}"
}

# IAM Role for the Put Lambda Function
...
# IAM Policy for the create/put log group/stream for the put function
resource "aws_iam_policy" "put_lambda_logging_policy" {
  name = "put_policy"
  path = "/"
  description = "IAM policy for logging from put lamdba"
